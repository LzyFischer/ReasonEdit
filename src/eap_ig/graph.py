# ============================== graph.py ==============================
# (Drop this into graph.py or keep in the same file with a guard.)
...
# NOTE: If you don't have visualization helpers, stub them or remove to_image.
try:
    from .visualization import get_color, generate_random_color
except Exception:  # minimal stubs
    def get_color(qkv, score):
        return "black"
    def generate_random_color(_):
        import random
        return f"#{random.randint(0,0xFFFFFF):06x}"

class Node:
    """A node in our computational graph. See original docstring."""
    name: str
    layer: int
    in_hook: str
    out_hook: str
    ...
    def score(self, value):
        if self.graph.nodes_scores is None:
            raise RuntimeError("Graph has no node scores enabled")
    ...
    def neurons(self, value):
        if self.graph.neurons is None:
            raise RuntimeError("Graph has no neuron storage enabled")
    ...
    def neurons_scores(self, value):
        if self.graph.neurons_scores is None:
            raise RuntimeError("Graph has no neuron score storage enabled")
...
class LogitNode(Node):
    ...
    @in_graph.setter
    def in_graph(self, value):
        raise ValueError("logits node always in graph")
...
class Edge:
    ...
    def __init__(self, graph: 'Graph', parent: Node, child: Node, qkv:Optional[Literal["q","k","v"]]=None):
        ...
        if isinstance(child, AttentionNode):
            if qkv is None:
                raise ValueError("Edges to attention heads must specify q/k/v")

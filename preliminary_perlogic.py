...
Path("output/perlogic").mkdir(parents=True, exist_ok=True)
df_acc    .to_csv("output/perlogic/accuracy.csv",  float_format="%.4f")
df_total  .to_csv("output/perlogic/n_total.csv")
df_correct.to_csv("output/perlogic/n_correct.csv")

# save the tag ↔︎ prompt lookup table
pd.DataFrame({"tag": logic_tags, "prompt": logic_types}) \
  .to_csv("output/perlogic/logic_tags.csv", index=False)
